...
export default function WeatherDashboard() {
  ...
  // All translatable UI text
  const [uiText, setUiText] = useState({
    title: 'Local Weather Hub',
    subtitle: 'Real-time farming conditions',
    selectLocation: 'Select Location',
    forecast: '7-Day Forecast',
    today: 'Today',
    tomorrow: 'Tomorrow',
    advice: 'Farming Advice Today',
    planting: 'Planting Window Status',
    ...
    footer: 'Weather data updates every hour. Forecasts and advice are tailored for agricultural activities.',
  });
  ...
  useEffect(() => {
    const loadWeather = async () => {
      ...
      try {
        ...
        const data = await getWeatherData(location.lat, location.lon, locationName);
        setWeatherData(data);
      } catch (error) {
  ...
  useEffect(() => {
    const handleLanguageChange = () => {
      setUiText({
        ...
        selectLocation: t('weatherSelectLocation'),
        forecast: t('weatherForecast'),
        today: t('weatherToday'),
        tomorrow: t('weatherTomorrow'),
        advice: t('weatherAdvice'),
        planting: t('weatherPlanting'),
        current: t('weatherCurrent'),
        temperature: t('weatherTemperature'),
        feelsLike: t('weatherFeelsLike'),
        humidity: t('weatherHumidity'),
        windSpeed: t('weatherWindSpeed'),
        alerts: t('weatherAlerts'),
        footer: t('weatherFooter'),
      });
    };
    ...
    <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl shadow-xl p-6 border border-blue-100">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{uiText.title}</h2>
            <p className="text-sm text-gray-600">{uiText.subtitle}</p>
          </div>
        </div>

        {/* Location Picker - Removed duplicate language selector */}
        <button
      ...
      {showLocationPicker && (
        <div className="mb-6 bg-white rounded-xl p-4 border border-blue-200">
          <h3 className="font-semibold text-gray-900 mb-3">{uiText.selectLocation}</h3>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
                ...
                >
                  <div className="text-center">
                    <p className="text-sm font-semibold text-gray-900 mb-2">
                      {index === 0 ? uiText.today : index === 1 ? uiText.tomorrow : day.day}
                    </p>
                    <div className="flex justify-center mb-2">{getWeatherIcon(day.icon, 'small')}</div>
        ...
        <div className="space-y-6">
          {/* Current Conditions */}
          <div className="bg-white rounded-xl p-6 border border-blue-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{uiText.current}</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <ThermometerSun className="w-5 h-5 text-orange-500" />
                  <span className="text-sm text-gray-600">{uiText.temperature}</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">{uiText.feelsLike}</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">{uiText.humidity}</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">{uiText.windSpeed}</span>
                </div>
      ...
      <div className="mt-6 bg-blue-50 rounded-lg p-4 border border-blue-200">
        <p className="text-xs text-gray-600 text-center">{uiText.footer}</p>
      </div>

      {/* Removed overlay for language menu since we're using the global language selector */}
    </div>
  );
}